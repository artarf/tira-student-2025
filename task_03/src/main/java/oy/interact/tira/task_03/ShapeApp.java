...
public class ShapeApp extends JPanel implements ShapesListener, ActionListener {
    ...
    private void run() throws ShapeException {
        ...
        shapesMenu.add(commandMenu);
        shapesMenu.addSeparator();
        commandMenu = new JMenuItem("Remove gray shapes BUGGY algo 1");
        commandMenu.setActionCommand("remove-gray-1");
        commandMenu.addActionListener(this);
        shapesMenu.add(commandMenu);
        commandMenu = new JMenuItem("Remove gray shapes SLOW algo 2");
        commandMenu.setActionCommand("remove-gray-2");
        commandMenu.addActionListener(this);
        shapesMenu.add(commandMenu);
        commandMenu = new JMenuItem("Remove gray shapes BIT FASTER algo 3");
        commandMenu.setActionCommand("remove-gray-3");
        commandMenu.addActionListener(this);
        shapesMenu.add(commandMenu);
        commandMenu = new JMenuItem("Remove gray shapes by PARTITIONING algo 4");
    ...
    private void drawSmallShapesCollection(Graphics g) {
        ...
        g.drawRect(originX, originY, totalWidth, cellHeight);
        ...
        String [] status = shapes.getRemovalStatus();
        g.drawString(status[0], originX, originY + 2 * cellHeight + 20);
        g.drawString(status[1], originX, originY + 3 * cellHeight + 20);
        g.drawString(status[2], originX, originY + 4 * cellHeight + 20);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            ...
            if ("18-shapes".equals(command)) {
                ...
                shapes.removeAndShiftDown(shapes.count() / 2);
                ...
                shapes.select(shape -> shape.getLineColor() != Color.GRAY);
                long start = System.currentTimeMillis();
                shapes.removeNotSelected3();
                System.out.println("removeSelected3 took " + (System.currentTimeMillis() - start) + " ms");
                shapes.invariant();
                repaint();
            } else if ("remove-gray-4".equals(command)) {
                System.out.format("Shapes array capacity before removing: %d%n", shapes.capacity());
                shapes.select(shape -> shape.getLineColor() != Color.GRAY);
                long start = System.currentTimeMillis();
                shapes.removeNotSelected4();
                System.out.println("removeSelected4 took " + (System.currentTimeMillis() - start) + " ms");
                System.out.format("Shapes array capacity after removing: %d%n", shapes.capacity());
                shapes.invariant();
                repaint();
            }
    ...
    public void exceptionHappened(String reason) {
        SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(frame, "Exception happened because: " + reason,
