...
/**
 * MazePanel takes care of drawing the maze and handling arrow key presses in
 * game mode.
 *
...
public class MazePanel extends JPanel implements MazeObserver, KeyListener {
    ...
    public MazePanel(Maze maze) {
        try {
            targetImage = ImageIO.read(ClassLoader.getSystemResource("Target.png"));
            monsterImage = ImageIO.read(ClassLoader.getSystemResource("Monster.png"));
        } catch (Exception e) {
            // Could not load image resource, failing silently. Draw dot instead.
            System.err.println(
                    "Could not load image resource, failing silently. Draw dot instead: " + e.getLocalizedMessage());
        }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int y = 0; y < maze.getHeight(); y++) {
            for (int x = 0; x < maze.getWidth(); x++) {
                Vertex<MazeNode> vertex = maze.getGraph().getVertexFor(new MazeNode(x, y));
                if (null != vertex) {
                    ...
                    List<EdgeInformation> neighbours = maze.getNeighbourDirections(x, y);
                    Stroke oldStroke;
                    ...
                    for (EdgeInformation edge : neighbours) {
                        switch (edge.direction) {
                            case EAST:
                                ...
                                graphics.draw(new Line2D.Float(vertexX + VERTEX_SIZE, vertexY + VERTEX_SIZE / 2,
                                        vertexX + edgeLengthWidth, vertexY + VERTEX_SIZE / 2));
                                graphics.setStroke(oldStroke);
                                if (showEdges && Math.abs(edge.weight - 1.0) > 0.1) {
                                    graphics.drawString(String.valueOf(edge.weight), vertexX + edgeLengthWidth / 2,
                                            vertexY - 2);
                                }
                                break;
                            case SOUTH:
                                oldStroke = graphics.getStroke();
                                graphics.setStroke(new BasicStroke(edge.weight));
                                graphics.draw(new Line2D.Float(vertexX + VERTEX_SIZE / 2, vertexY + VERTEX_SIZE,
                                        vertexX + VERTEX_SIZE / 2, vertexY + edgeLengthHeight));
                                graphics.setStroke(oldStroke);
                                if (showEdges && Math.abs(edge.weight - 1.0) > 0.1) {
                                    graphics.drawString(String.valueOf(edge.weight), vertexX + VERTEX_SIZE + 3,
                                            vertexY + edgeLengthHeight / 2);
                                }
                                break;
                    ...
                    if (vertex.equals(maze.getStart())) {
                        if (null != monsterImage) {
                            g.drawImage(monsterImage,
                                    vertexX - (IMAGE_SIZE / 2),
                                    vertexY - (IMAGE_SIZE / 2),
                                    IMAGE_SIZE,
                                    IMAGE_SIZE, this);
                        } else {
                            Color current = g.getColor();
                            g.setColor(Color.BLUE);
                            g.fillOval(vertexX - VERTEX_SIZE / 2, vertexY - VERTEX_SIZE / 2, VERTEX_SIZE * 2,
                                    VERTEX_SIZE * 2);
                            g.setColor(current);
                        }
                        ...
                        if (null != targetImage) {
                            ...
                            g.fillRect(vertexX - VERTEX_SIZE / 2, vertexY - VERTEX_SIZE / 2, VERTEX_SIZE * 2,
                                    VERTEX_SIZE * 2);
                            g.setColor(current);
                        }
                    }
